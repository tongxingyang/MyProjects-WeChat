...
export default class Hurdle extends Laya.Script {
    ...
    onUpdate() {
        ...
        if (Math.abs(myPos.x - pPos.x) <= 1.5 && Math.abs(myPos.z - pPos.z) <= 0.1) {
            ...
            if (this.type == 1) {
                //竖
                if (this.num.substring(0, 1) == '+' || this.num.substring(0, 1) == '*') SoundMgr.instance.playSoundEffect('green')
                if (this.num.substring(0, 1) == '-' || this.num.substring(0, 1) == '/') SoundMgr.instance.playSoundEffect('red')
