...
export default class FdAd {
    ...
    static inidAd(cb?: Function) {
        ...
        let func = () => {
            if (this.isFullGridAdLoaded && this.bannerLoadedCount >= this.bannerIdArr.length) {
    ...
    static bannerLoadedCount: number = 0
    static initBanner() {
        if (!Laya.Browser.onWeiXin) return;
    ...
    static showBannerAd(isCountTime: boolean = true) {
        if (!Laya.Browser.onWeiXin) return;
