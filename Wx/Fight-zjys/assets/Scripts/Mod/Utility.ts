import { _decorator, Sprite, SpriteFrame, SpriteAtlas, Node, Vec2, v2, resources, UITransform, Vec3, Quat, v3, find, tween, Rect } from "cc";
...
export default class Utility {
    ...
    public static getBoundingBoxToWorld(node: Node): Rect {
        let r = new Rect()
        let size = node.getComponent(UITransform).contentSize.clone()
        let anchor = node.getComponent(UITransform).anchorPoint.clone()
        let wPos = node.getComponent(UITransform).convertToWorldSpaceAR(v3())
        r.x = wPos.x - size.width * anchor.x
        r.y = wPos.y - size.height * anchor.y
        r.width = size.width
