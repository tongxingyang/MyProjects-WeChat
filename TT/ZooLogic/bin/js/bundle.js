(function () {
    ...
    class GameLogic {
        constructor() {
            ...
            AdMgr.initAd();
            if (Laya.Browser.onTTMiniGame) {
                Laya.Browser.window.tt.showShareMenu({});
        ...
        gameOver(isWin) {
            ...
            Laya.timer.once(2000, this, () => {
                ...
                Laya.Scene.open('MyScenes/ShareRecordUI.scene', false, {
                    ccb: () => {
    ...
    class LoadingUI extends Laya.Scene {
        ...
        loadSubpackage() {
            const loadTask = Laya.Browser.window.tt.loadSubpackage({
                name: 'unity',
                success: (res) => {
                    this.loadRes();
                },
    ...
    class SelectUI extends Laya.Scene {
        ...
        selectItemUp(type, index) {
            console.log('click');
            if (PlayerDataMgr.getDataByType(type)[index] == 0) {
                GameRecordMgr.recordPause();
                let cb = () => {
                    PlayerDataMgr.getDataByType(type)[index] = 1;
                    PlayerDataMgr.setPlayerData();
                    this.updateItemList();
                };
                AdMgr.showVideoAd(cb, () => {
                    GameRecordMgr.recordResume();
                });
                return;
            }
        }
    ...
    class ShareRecordUI extends Laya.Scene {
        constructor() {
            super();
            this.ccb = null;
        }
