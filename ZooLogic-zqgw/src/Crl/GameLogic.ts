...
export default class GameLogic {
    ...
    onLoadScene(scene) {
        ...
        if (!FdMgr.isVersionValid) {
            let s1 = this._scene.getChildByName('Scene_1') as Laya.Sprite3D
            for (let i = 0; i < s1.numChildren; i++) {
                let s = s1.getChildAt(i) as Laya.MeshSprite3D
                let mat = s.meshRenderer.material as Laya.BlinnPhongMaterial
