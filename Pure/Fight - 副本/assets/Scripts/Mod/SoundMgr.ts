...
export class SoundMgr extends Component {
    ...
    start() {
        // [3]
        this.innerAudioContext = window['wx'].createInnerAudioContext()
    }
    ...
    PlaySound(key: string, loop: boolean = false, volume: number = 1) {
        ...
        if (WECHAT) {
            this.innerAudioContext.src = this.node.getChildByName(key).getComponent(AudioSource).clip.nativeUrl;
