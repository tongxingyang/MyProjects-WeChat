    ...
    for (let i = 0; i < 2; i++) {
      let grid = wx.createCustomAd({
        ...
        style: {
          ...
          top: sysInfo.screenHeight * 0.55
        }
      });
      grid.onError((err) => {
        console.log('屏幕单格子加载失败:', JSON.stringify(err));
      })
      grid.onLoad(() => {
        this.singleGridAd.push(grid);
      })
    }
  },
  ...
  createSideGrid() {
    ...
    for (let i = 0; i < 2; i++) {
      let grid = wx.createCustomAd({
        ...
        style: {
          left: i == 0 ? 0 : sysInfo.screenWidth - 65,
          top: i == 0 ? sysInfo.screenHeight * 0.5 : sysInfo.screenHeight * 0.3
        }
      });
      grid.onError((err) => {
        console.log('屏幕侧格子加载失败:', JSON.stringify(err));
      })
      grid.onLoad(() => {
